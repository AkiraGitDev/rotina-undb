...
import { currentUser, mockProjects, tasksOfUser } from '@/lib/mock';

export default function MinhasTarefasScreen() {
  const router = useRouter();
  const tarefas = tasksOfUser(currentUser.id);
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            ...
            {ativas.map((t) => (
              ...
              >
                ...
                <Text style={styles.meta}>{mockProjects.find((p) => p.id === t.projetoId)?.nome}</Text>
              </ListItem>
            ))}
          </View>
        )}
        ...
        {concluidas.length > 0 && (
          <View style={styles.section}>
            ...
            {concluidas.map((t) => (
              <ListItem key={t.id} leading={<View style={[styles.checkbox, styles.checkboxDone]} />}>
